"""
Crystal detector — Cellpose version (screen capture mode).

Cellpose is a microscopy-specialist segmentation model that works without
any reference image — it segments objects directly from each frame.

Workflow:
  1. Run the script.
  2. Drag a rectangle over the NIS-Elements live view window.
  3. Cellpose segments crystals in real-time with adjustable parameters.

Usage:
    python crystal_cellpose_detector.py
...
# ─────────────────────────────────────────────────────────────────────────────
# Centroid tracker (full version with velocity + graveyard)
# ─────────────────────────────────────────────────────────────────────────────
...
class SliderPanel:
    ...
    def render(self):
        ...
        cv2.putText(c,"CELLPOSE — LIVE SCREEN CAPTURE",(10,self.SCRUB_H//2+6),
...
def overlay_masks(display, masks, tracked):
    """Draw Cellpose masks + tracker IDs."""
...
def draw_hud(display, n, total, fps, paused, diameter, model_type):
    hud=display.copy(); cv2.rectangle(hud,(6,6),(420,150),(0,0,0),-1)
    cv2.addWeighted(hud,0.45,display,0.55,0,display)
    cv2.putText(display,f"CRYSTAL DETECTOR  [Cellpose — {model_type}]",(14,30),
    ...
    cv2.putText(display,"r=reselect region  s=save  SPACE=pause  q=quit",(14,130),
                cv2.FONT_HERSHEY_SIMPLEX,0.38,(120,120,140),1)


# ─────────────────────────────────────────────────────────────────────────────
# Cellpose inference (runs in background thread)
# ─────────────────────────────────────────────────────────────────────────────
def masks_from_cellpose(model, frame_rgb, diameter, flow_threshold, cellprob_threshold,
                        min_area, max_area):
    """Run Cellpose and return list of (H,W) bool masks."""
    ...
    with mss.MSS() as sct:
        while True:
            ...
            if not paused:
                ...
                if masks:
                    # Scale masks to display size
                    scaled = [cv2.resize(m.astype(np.uint8),(dw,dh),
                              interpolation=cv2.INTER_NEAREST).astype(bool)
                              for m in masks]
                    # Scale tracked centroids
                    sx,sy = dw/rw, dh/rh
            ...
            diameter = params[0]['value']
            draw_hud(display,len(shared['masks']),tracker.total_seen,fps_val,paused,diameter,model_type)
            combined=np.vstack([display,panel.render()])
...
def run():
    ...
    # Cellpose model — 'cyto3' is best for round/irregular objects under microscope
    # Options: 'cyto3' (best accuracy), 'cyto2', 'cyto', 'nuclei'
    model_type = 'cyto3'
    print(f"Loading Cellpose model: {model_type} ...")
    model = models.CellposeModel(gpu=(device=='cuda'), model_type=model_type)
    print("Model loaded.")
    ...
    while True:
        ...
        tracker.reset()
        result = detection_loop(model, model_type, region, params, tracker)
        print(f"Session: {tracker.total_seen} unique crystals tracked.")
        if result == 'quit': break
        # 'reset' → loops back to region selector
